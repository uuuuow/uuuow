...
// 网站的根目录
const char* doc_root = "/home/now/socket/resources";
...
void http_conn::init()
{
    ...
    m_host = 0;
    m_write_idx = 0;
    memset(m_read_buf,0,READ_BUFFER_SIZE); // 或者用bzero(m_read_buf,READ_BUFFER_SIZE);
...
http_conn::HTTP_CODE http_conn::parse_request_line(char *text){
    ...
    *m_version++ = '\0';
    ...
    if(strncasecmp(m_url,"http://",7) == 0){
        m_url += 7;
        m_url = strchr(m_url,'/'); // strchr（）函数，找到第一个为"/"的下标
    }
    if(!m_url || m_url[0] != '/'){  // 找index前的"/" 
        return BAD_REQUEST;
    }
    m_check_state = CHECK_STATE_HEADER; // 主机状态变成检查头
    return NO_REQUEST;
}
...
http_conn::HTTP_CODE http_conn::parse_headers(char *text){
    // 遇到空行情况
     if(text[0] == '\0'){
        ...
        // 处理// 处理Content-Length头部字段 Connection: keep-alive
        text += 11;
        text += strspn(text," \t"); // strspn()函数，找到不属于" \t"的第一个下标
